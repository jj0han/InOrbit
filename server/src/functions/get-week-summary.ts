...
export async function getWeekSummary() {
  ...
  const result = await db
    .with(goalCompletedInWeek, goalCreatedUpToWeek, goalsCompletedByWeekDay)
    .select({
      completed:
      ...
      total:
        sql/*sql*/ `(SELECT SUM(${goalCreatedUpToWeek.desiredWeeklyFrequency}) FROM ${goalCreatedUpToWeek})`.mapWith(
          Number
        ),
